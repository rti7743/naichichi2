...
//画像読み込み
bool XLImage::Load(const string & fileName)
{
...
//画像読み込み
bool XLImage::Load(const vector<char> & data)
{
	Clear();
...
//画像保存
bool XLImage::Save(const string & fileName,int option) const
{
	

	assert(IsEnable()) ;
	string ext = XLStringUtil::strtolower(XLStringUtil::baseext(fileName));
	...
	if (ext == ".jpeg" || ext == ".jpg")
	{
		...
		params.Parameter[0].NumberOfValue	= 1;
...
//画像保存
bool XLImage::Save(const string & ext,vector<char> * data,int option) const
{
	assert(IsEnable()) ;
...
bool XLImage::findEncoder(const string & ext,CLSID* clsid) const
{
...
bool XLImage::ConvertHBITMAPtoBytes(HBITMAP hbitmap,vector<char> * image) const
{
	BITMAP bmp; 
