...
EndelS::EndelS()
{
	...
	this->EndelType = EndelType_UArt0;
}
...
void EndelS::Create()
{
	const string type = MainWindow::m()->Config.Get("endel_type","uart0");
	if (type == "" || type == "uart0")
	{
		this->EndelType = EndelType_UArt0;
	}
...
int EndelS::getLumi()
{
	if (this->EndelType == EndelType_None) return 0;
...
int EndelS::Capture(IRData * irdata)
{
	if (this->EndelType == EndelType_None) return 0;
...
int EndelS::Radiate(const IRData * irdata)
{
	if (this->EndelType == EndelType_None) return 0;
...
void EndelS::DumpIRData(const IRData * irdata)
{
	printf("==IR DATA==\r\n");
...
bool EndelS::SaveIRData(const IRData * irdata,const string& filename)
{
...
bool EndelS::LoadIRData(IRData * irdata,const string& filename)
{
...
bool EndelS::checkIR(const IRData * irdata)
{
	//データが少なすぎる 4の根拠はない。
...
void EndelS::TurnOnLED()
{
	if (this->EndelType == EndelType_None) return ;
