...
cpick = function (trigger, target, p) {
	...
	//
//	if( ! /^input$/i.test(target.nodeName) || ! /^(text|hidden)$/.test(target.type) ) { return; }
//	if( ! /^(input|button|a|div)$/i.test(trigger.nodeName) ) { return; }
	/* -------------------------------------------------------------------
	* default settings
	* ----------------------------------------------------------------- */
	var dp = {
...
proto.show = function() {
	...
	if (this.p["posstion"] == "bottom") {
		panel.style.top = (pos.top + pos.height) + "px";
		panel.style.left = pos.left + "px";
	}
	else if (this.p["posstion"] == "up") {
		panel.style.top = (pos.top - this.p["height"]) + "px";
		panel.style.left = pos.left + "px";
	}
	else
	{
...
proto._create_panel = function() {
	...
	var footer_control_height = Math.floor(footer_height * 0.8);
	// footer box
	...
	this._set_styles(ft, {
		...
		height: footer_control_height + "px",
		whiteSpace: "nowrap"
	});
	...
	_add_event_listener(this.nodes.tbox, "keydown", function(e) { _this.on_edittbox_keyup(e); });
	_add_event_listener(this.nodes.tbox, "keyup", function(e) { _this.on_edittbox_keyup(e); });
	//
	return panel;
};

proto.on_edittbox_keyup = function(e) {

	var rgb = this._conv_color_to_rgb(this.nodes.tbox.value);
	if( rgb != null ) {
		this.rgb = rgb;
		this.hls = this._rgb_to_hls(rgb);
		...
		var color = this._conv_rgb_to_css_hex(rgb);
...
//	}
//	tstyle.backgroundColor = this._conv_rgb_to_css_hex(this.rgb);
	tstyle.color = this._conv_rgb_to_css_hex(this.rgb);
};
...
proto._ok_clicked = function(e) {
	...
	if ( this.p["callback"] ){
		this.p["callback"]( this._conv_rgb_to_css_hex(this.rgb) );
	}
	this.hide();
...
//	var msy = scroll_top + e.clientY;
	...
	{
		msx = scroll_left + e.changedTouches[0].pageX;
		msy = scroll_top + e.changedTouches[0].pageY;
	}
	else
	{
		msx = scroll_left + e.clientX;
		msy = scroll_top + e.clientY;
	}
	var x = msx - elx;
	var y = msy - ely;
	return {x:x, y:y};
...
var cpicks = [];
_add_event_listener(window, "load", function(){
...
function _add_event_listener(elm, type, func) {
	...
	var subtype = null;
	if (type == "mousedown") subtype = "touchstart";
	else if (type == "mousemove") subtype = "touchmove";
	else if (type == "mouseup") subtype = "touchend";
	
	if(elm.addEventListener) {
		elm.addEventListener(type, func, false);
		if (subtype) elm.addEventListener(subtype, func, false);
	} else if(elm.attachEvent) {
		elm.attachEvent('on'+type, func);
		if (subtype) elm.attachEvent('on'+subtype, func);
		/*
...
function new_cpick(elem,option) {
	var target = elem;
	if (option["target"]) {
	...
	// color picker object
	var o = new cpick(elem, target, option);
	if( ! o ) { return; }
	o.prepare();
	return o;
}
