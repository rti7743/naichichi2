...
void Recognition_SAPI::Callback(WPARAM wParam, LPARAM lParam)
{
	...
	{
		this->PoolMainWindow->SyncInvokeError( e.what() );
	}
...
private:
	...
	public:
		...
		list<const SPPHRASERULE*> pickupRules;
...
//			this->Track.track(this->Phrase->Rule.pFirstChild->pNextSibling);
	...
	map<string , string> GetRegexpCapture() const
	{
...
bool Recognition_SAPI::checkDictation(const string & dictationString) const
{
	auto it = this->YobikakeListArray.begin();
	for( ; this->YobikakeListArray.end() != it ; ++it)
	{
		//ディクテーションフィルターで絞る
		if ( dictationString.find( *it ) != string::npos )
		{
			break;
		}
	}
...
//これで過剰なマッチを排除します。
string Recognition_SAPI::convertDictation(ISpRecoResult* result,const string& ruleName)
{
	HRESULT hr;
...

		{
			...
			{//認識した結果
				ISpRecoResult* tempresult;
				{
					...
					string ret = _W2A(tempdstrText);
					this->PoolMainWindow->SyncInvokeLog(string() + "ディクテーションフィルター :" + ret + + " " + num2str(confidence),LOG_LEVEL_DEBUG);

					if (confidence <= 0.60)
					{
						this->PoolMainWindow->SyncInvokeLog(string() + "ディクテーションフィルター棄却",LOG_LEVEL_DEBUG);
						return "";
					}
...
bool Recognition_SAPI::DebugSaveWavFile(const string& directory,ISpStreamFormat* streamFormat) const
{
	HRESULT hr;
...

	{
		CSpStreamFormat OriginalFmt;
		{
			...
			CComPtr<ISpStream> cpStream;
			{
				...
				string fff = directory + "\\" + num2str(time(NULL))+".wav";
...
bool Recognition_SAPI::SetYobikake(const list<string> & yobikakeList)
{
...
bool Recognition_SAPI::SetRecognitionFilter(double yobikakeRuleConfidenceFilter,double basicRuleConfidenceFilter,bool useDictationFilter)
{
...
bool Recognition_SAPI::AddCommandRegexp(const CallbackDataStruct * callback,const string & str)
{
...
//メディア情報をアップデートします。
bool Recognition_SAPI::UpdateMedia(const string& name ,const list<string>& list )
{
return true;
/*
...
bool Recognition_SAPI::AddRegexp(unsigned int id,const string & str ,SPSTATEHANDLE stateHandle ) 
{
	...
	for( const wchar_t * p = _A2W( str.c_str() ) ; *p ; ++p )
	{
        ...
        else if ( *p == L'[' && *(p+1) == L':' )
        {
			...
			optstr += wstring(p , (end - p) + 2);
			p = end + 1;
        }
        else if (*p == L'*' || *p == L'+' || *p == L'.' || *p == L'[' || *p == L']')
        {
			throw XLException(string("") + "現在は、メタ文字 " + _W2A(p) + " は利用できません。利用可能なメタ文字 () | .+ ?)");
        }
        else
        {
            optstr += *p;
...
bool Recognition_SAPI::AddRegexpImpl(const SPPROPERTYINFO* prop,const wstring & str, SPSTATEHANDLE stateHandle)
{
	...
	for(  ; *p ; ++p )
	{
		if (*p == L'(')
        {
            ...
            //ネストする前の部分
            matchString = wstring(splitPos, 0 ,p - splitPos);
            ...
            //ネストしているルールを再帰して実行.
			matchString = wstring(p+1 , 0 , (int) (n - p - 1) );
        ...
        {
        	matchString = wstring(splitPos,0 , (int) (p - splitPos));
...
/*
			wstring spname = wstring(p+2,0,end - (p + 2));
			SPSTATEHANDLE targetHandle = NULL;
			if (spname == L"music")
			{
				targetHandle = this->MusicRuleHandle;
			...
			{
				throw XLException(string() + "[: " + _W2A(spname.c_str()) + " :] にマッチするルールはありません");
			}
