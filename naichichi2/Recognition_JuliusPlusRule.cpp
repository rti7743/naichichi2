...
{
	remove_if(this->nestrules.begin(),this->nestrules.end() ,
...
//読みからマッチする単語構造体を取得する
const Recognition_JuliusPlusRule::wordnode* Recognition_JuliusPlusRule::findYomi(const string& yomiPress) const
{
...
//読みを作成する。
string Recognition_JuliusPlusRule::convertYomi(const string& word) 
{
	//カタカナひらがな変換
	string w = MainWindow::m()->MecabControl.KanjiAndKanakanaToHiragana(word);
...
//空白なしにする
string Recognition_JuliusPlusRule::converPress(const string& word) 
{
	return XLStringUtil::replace(word, " " , "");
}

//読みを作成する(空白なし)
string Recognition_JuliusPlusRule::convertYomiAndPress(const string& word) 
{
