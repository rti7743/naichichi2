...
public:
	...
	//画像読み込み
	bool Load(const vector<char> & data);
	//画像読み込み
	...
	//画像保存
	bool Save(const string & fileName,int option = INT_MAX) const;
	//画像保存
	bool Save(const string & ext,vector<char> * data,int option) const;
...
private:
	void Clear();
	bool findEncoder(const string & ext,CLSID* clsid) const;
	bool IsEnable() const
	{
