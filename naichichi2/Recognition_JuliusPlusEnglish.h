...
class Recognition_JuliusPlusEnglish: public ISpeechRecognitionInterface
{
public:
	enum 
	{
	...
	Recognition_JuliusPlusEnglish();
	virtual ~Recognition_JuliusPlusEnglish();
...
private:
	...
	//フレームの取得
	bool getRecongFrame3(const SentenceAlign * align , int i_seq , unsigned int* retBeginFrame , unsigned int* retEndFrame, unsigned int* retEndFrame2) const;
	...
	//連続した読みを取得する
	string ConvertJuliusYomiJoin(const WORD_ID* seq,int seqIndex,const WORD_INFO * wordinfo,const LOGPROB* confidence,LOGPROB* avgConfidence) const;
	...
	//読みを作成する。
	string ConvertYomiNoJapanesWakachigaki(const string& word) const;

	list<string> YobikakeListArray;
	string YobikakeFirstKanaChar;
	string YobikakeSecondKanaChar;
