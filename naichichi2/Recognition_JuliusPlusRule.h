...
public:
	struct wordnode
	{
		wordnode(const string& word,const string& yomi) : word(word) , yomi(yomi)
		{
...
private:
	...
	list<Recognition_JuliusPlusRule*> nestrules;
...
public:
	...
	bool AddWord(const string& w , const string& yomi )
	{
	...
	const list<wordnode*>* getWords() const
	{
		return &this->words;
	}
	const list<Recognition_JuliusPlusRule*>* getNestRules() const
	{
	...
	//読みからマッチする単語構造体を取得する
	const wordnode* findYomi(const string& yomiPress) const;
	//このノードは自分の下位ノードですか？ (term除く)
	bool IsMemberNode(const Recognition_JuliusPlusRule* node,bool nest = false) const;
	//読みを作成する。
	static string convertYomi(const string& word) ;
	//読みを作成する(空白なし)
	static string convertYomiAndPress(const string& word) ;
	//空白なしにする
	static string converPress(const string& word) ;
};
