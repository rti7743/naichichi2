...
// 書き込み
static int ir_proc_recv_write( struct file *filp, const char *buf, unsigned long len, void *data )
{
	//常に失敗.
	return -EBUSY;
}
...
// 読み込み
static int ir_proc_recv_read( char *page, char **start, off_t offset, int count, int *eof, void *data )
{
	unsigned int copysize ;
	
	if (offset >= IRRecvBufferIndex*4)
	{
		printk(KERN_ALERT "read EOF IRRecvBufferIndex*4:%d offset:%d\n",IRRecvBufferIndex*4,offset);
		
		*eof = 1;
		return 0;
	}

	if (IRRecvBufferIndex*4 <= 0)
	{
		printk(KERN_ALERT "can not alloc output memory \n");
	...
	//コピーするサイズを計算します.
	copysize = IRRecvBufferIndex*4 - offset;
	if (copysize > count) copysize = count;

	printk(KERN_ALERT "read IRRecvBufferIndex*4:%d offset:%d copysize:%d  \n",IRRecvBufferIndex*4,offset,copysize);

	//実際のコピー
	memcpy(page,((char*)IRRecvBuffer) + offset,copysize);

	//次もまた必ずコールしてもらえるように、コピーしたサイズをそれぞれに返します。
	*start = (char*)copysize;
	return (int) copysize;
}

// 書き込み
static int ir_proc_recv_count_write( struct file *filp, const char *buf, unsigned long len, void *data )
{
	//常に失敗.
	return -EBUSY;
}
...
// 読み込み 現在のバッファの位置を返します.
static int ir_proc_recv_count_read( char *page, char **start, off_t offset, int count, int *eof, void *data )
{
	return sprintf(page,"%u",IRRecvBufferIndex);
}
...
static int irr_driver_init(void)
{
	...
	entry = create_proc_entry( IR_PROCNAME_RECV, 0666, NULL );
	if( entry ){
		entry->write_proc  = ir_proc_recv_write;
		entry->read_proc  = ir_proc_recv_read;
	}
	else{
	...
	entry = create_proc_entry( IR_PROCNAME_RECV_COUNT, 0666, NULL );
	if( entry ){
		entry->write_proc  = ir_proc_recv_count_write;
		entry->read_proc  = ir_proc_recv_count_read;
	}
	else{
	...
	err = request_any_context_irq(
		...
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_NOBALANCING | IRQF_IRQPOLL | IRQF_DISABLED,
		DRV_NAME,
		NULL
	);
	if (err < 0) {
