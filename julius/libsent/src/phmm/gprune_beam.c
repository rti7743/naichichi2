 ...
 *
 * gprune_beam()は混合ガウス分布集合の計算･・璽船鵑琉・弔任后･
 * beam pruning を使って上位のガウス分布の出力確率のみを高速に求めます．
 * Tied-mixture %HMM 使用時に Julius でGPRUNE_DEFAULT_BEAM がﾄ・舛気・討い・��､
 * あ､・いﾏJuliusのオプショ･・"-gprune beam" を指定す､・海箸任海隆愎瑤ｬ
 * 使用さ､・泙后･
 *
 * beam pruning は最も積極的に枝刈りを行ないます．計算は最も高速ですが，
 * 上位のガウス分布が正しく得ら､・砂侘漏領┐慮�占が大きくなぁδ能性があります�･
 * 
 * gprune_beam() は outprob_init() によってその関数へのポインタが
 * compute_gaussset にセットさ､・・海箸濃藩僂気・泙后イ海離櫂ぅ鵐燭ｬ
 * calc_tied_mix() または calc_mix() から呼び出さ､・泙后･
 * </JA>
 * 
 * <EN>
