...
struct adinopt_freebsd {
	int srate;		///< Required sampling rate
	int audio_fd;		///< Audio descriptor
	boolean need_swap;	///< Whether input samples need byte-swapping
	pollfd fds[1];		///< Workarea for polling device

	char *defaultdev = DEFAULT_DEVICE; ///< Default device name
	char devname[MAXPATHLEN];		///< Current device name
};
typedef struct adinopt_freebsd ADINOPT;
 ...
 */
static boolean
adin_mic_open(ADINOPT* adinopt,char *devstr)
{
  ...
  /* open device */
  if ((adinopt->audio_fd = open(devstr, O_RDONLY)) == -1) {
  ...
  /* and set fmt */
  if (ioctl(adinopt->audio_fd, SNDCTL_DSP_GETFMTS, &fmt_can) == -1) {
  ...
  if (close(adinopt->audio_fd) != 0) return FALSE;
  ...
  rfmt = fmt;
  if (ioctl(adinopt->audio_fd, SNDCTL_DSP_SETFMT, &rfmt) == -1) {
  ...
  stereo = 0;			/* mono */
  if (ioctl(adinopt->audio_fd, SNDCTL_DSP_STEREO, &stereo) == -1) {
  ...
  samplerate = adinopt->srate;
  if (ioctl(adinopt->audio_fd, SNDCTL_DSP_SPEED, &samplerate) == -1) {
    jlog("Erorr: adin_freebsd: failed to set sample rate to %dHz\n", adinopt->srate);
    return(FALSE);
  }
  if (samplerate < adinopt->srate - FREQALLOWRANGE || samplerate > adinopt->srate + FREQALLOWRANGE) {
    jlog("Error: adin_freebsd: failed to set sampling rate to near %dHz. (%d)\n", adinopt->srate, samplerate);
    return FALSE;
  }
  if (samplerate != adinopt->srate) {
    jlog("Warning: adin_freebsd: specified sampling rate was %dHz but set to %dHz, \n", adinopt->srate, samplerate);
  }

  /* set polling status */
  adinopt->fds[0].fd = adinopt->audio_fd;
  adinopt->fds[0].events = POLLIN;
 ...
 */
boolean
adin_mic_begin(void * adinoptTHIS,char *pathname)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  /* set device name */
  if (pathname != NULL) {
    strncpy(adinopt->devname, pathname, MAXPATHLEN);
    jlog("Stat: adin_freebsd: device name = %s (from argument)\n", adinopt->devname);
  } else if ((p = getenv("AUDIODEV")) != NULL) {
    strncpy(adinopt->devname, p, MAXPATHLEN);
    jlog("Stat: adin_freebsd: device name = %s (from AUDIODEV)\n", adinopt->devname);
  } else {
    strncpy(adinopt->devname, adinopt->defaultdev, MAXPATHLEN);
    jlog("Stat: adin_freebsd: device name = %s (application default)\n", adinopt->devname);
  }

  /* open the device */
  return(adin_mic_open(adinopt,adinopt->devname));
}
...
adin_mic_read(void * adinoptTHIS,SP16 *buf, int sampnum)
{
  ...
  poll(adinopt->fds, 1, POLLINTERVAL);
  /* get actual sample num in the device buffer */
  if (ioctl(adinopt->audio_fd, SNDCTL_DSP_GETISPACE, &info) == -1) {
  ...
  cnt = read(adinopt->audio_fd, buf, size);
  if ( cnt < 0 ) {
