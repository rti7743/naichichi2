...
struct adinopt_oss {
	...
	int frag_size;		///< Actual data fragment size
	boolean stereo_rec;	///< TRUE if stereo recording (use left only)
	char *defaultdev ; ///< Default device name
	char devname[MAXPATHLEN];		///< Current device name
};
typedef struct adinopt_oss ADINOPT;
...
adin_oss_standby(int sfreq, void *dummy)
{
  ...
  return NULL;
#else
  ADINOPT * ainopt = (ADINOPT*) mymalloc(sizeof(ADINOPT));
  memset(ainopt,0,sizeof(ADINOPT));
  /* store required sampling rate for checking after opening device */
  ainopt->srate = sfreq;
  ainopt->defaultdev = DEFAULT_DEVICE;
  return ainopt;
}
 ...
 */
static boolean
adin_oss_open(void * adinoptTHIS,char *devstr)
{
  ADINOPT* ainopt = (ADINOPT*)adinoptTHIS;
  ...
  /* open device */
  if ((ainopt->audio_fd = open(devstr, O_RDONLY|O_NONBLOCK)) == -1) {
  ...
  /* and set fmt */
  if (ioctl(ainopt->audio_fd, SNDCTL_DSP_GETFMTS, &fmt_can) == -1) {
  ...
  if (fmt_can & fmt1) {
    fmt = fmt1;
    ainopt->need_swap = FALSE;
  } else if (fmt_can & fmt2) {
    fmt = fmt2;
    ainopt->need_swap = TRUE;
  } else {
  ...
  if (close(ainopt->audio_fd) != 0) return FALSE;

  /* re-open for recording */
  /* open device */
  if ((ainopt->audio_fd = open(devstr, O_RDONLY)) == -1) {
  ...
  /* (and smaller fragment causes busy buffering) */
  {
    ...
    f = 0;
    f2 = 1;
    while (f2 * 1000 / (ainopt->srate * sizeof(SP16)) <= frag_msec
    ...
    /* set to device */
    arg = 0x7fff0000 | frag;
    if (ioctl(ainopt->audio_fd, SNDCTL_DSP_SETFRAGMENT, &arg)) {
      jlog("Stat: adin_oss: set fragment size to 2^%d=%d bytes (%d msec)\n", frag, 2 << (frag-1), (2 << (frag-1)) * 1000 / (ainopt->srate * sizeof(SP16)));
    }
  }
  
  /* set format, samplerate, channels */
  rfmt = fmt;
  if (ioctl(ainopt->audio_fd, SNDCTL_DSP_SETFMT, &rfmt) == -1) {
...

  {
    ...
    stereo = 0;			/* mono */
    if (ioctl(ainopt->audio_fd, SNDCTL_DSP_STEREO, &stereo) == -1) {
    ...
    if (! ok_p) {		/* not setup yet */
      ...
      channels = 1;
      if (ioctl(ainopt->audio_fd, SNDCTL_DSP_CHANNELS, &channels) == -1) {
  ...
  samplerate = ainopt->srate;
  if (ioctl(ainopt->audio_fd, SNDCTL_DSP_SPEED, &samplerate) == -1) {
    jlog("Erorr: adin_oss: failed to set sample rate to %dHz\n", ainopt->srate);
    return(FALSE);
  }
  if (samplerate < ainopt->srate - FREQALLOWRANGE || samplerate > ainopt->srate + FREQALLOWRANGE) {
    jlog("Error: adin_oss: failed to set sampling rate to near %dHz. (%d)\n", ainopt->srate, samplerate);
    return FALSE;
  }
  if (samplerate != ainopt->srate) {
  ...
  if (ioctl(ainopt->audio_fd, SNDCTL_DSP_GETBLKSIZE, &ainopt->frag_size) == -1) {
  ...
  jlog("Stat: adin_oss: audio I/O Latency = %d msec (fragment size = %d samples)\n", ainopt->frag_size * 1000/ (ainopt->srate * sizeof(SP16)), ainopt->frag_size / sizeof(SP16));
 ...
 */
boolean
adin_oss_begin(void * adinoptTHIS,char *pathname)
{
  ADINOPT* ainopt = (ADINOPT*)adinoptTHIS;
  char buf[4];
  char *p;
 ...
 */
int
adin_oss_read(void * adinoptTHIS,SP16 *buf, int sampnum)
{
  ADINOPT* ainopt = (ADINOPT*)adinoptTHIS;
#ifndef HAS_OSS
  return -2;
  ...
  FD_ZERO(&rfds);
  FD_SET(ainopt->audio_fd, &rfds);
  tv.tv_sec = 0;
  tv.tv_usec = MAXPOLLINTERVAL * 1000;
  status = select(ainopt->audio_fd+1, &rfds, NULL, NULL, &tv);
  if (status < 0) {
    /* select() failed */
    jlog("Error: adin_oss: failed to poll device\n");
    return(-2);			/* error */
  }
  if (FD_ISSET(ainopt->audio_fd, &rfds)) { /* has some data */
    /* get sample num that can be read without blocking */
    if (ioctl(ainopt->audio_fd, SNDCTL_DSP_GETISPACE, &info) == -1) {
    ...
    if (size < ainopt->frag_size) size = ainopt->frag_size;
    size &= ~ 1;		/* Force 16bit alignment */
    cnt = read(ainopt->audio_fd, buf, size);
    if ( cnt < 0 ) {
    ...
    if (ainopt->need_swap) swap_sample_bytes(buf, cnt);
 ...
 */
char *
adin_oss_input_name(void * adinoptTHIS)
{
#ifndef HAS_OSS
  return NULL;
#else
  ADINOPT* ainopt = (ADINOPT*)adinoptTHIS;
  return(ainopt->devname);
#endif
}
