 ...
 */
static boolean
adin_sndfile_open(ADINOPT* adinopt,char *filename)
{
#ifndef HAVE_LIBSNDFILE_VER1
  adinopt->sinfo.samplerate = adinopt->sfreq;
  adinopt->sinfo.pcmbitwidth = 16;
  adinopt->sinfo.channels = 1;
#endif
  adinopt->sinfo.format = 0x0;
  if ((adinopt->sp = 
#ifdef HAVE_LIBSNDFILE_VER1
       sf_open(filename, SFM_READ, &adinopt->sinfo)
#else
       sf_open_read(filename, &adinopt->sinfo)
#endif
       ) == NULL) {
    ...
    adinopt->sinfo.channels = 1;
#ifdef HAVE_LIBSNDFILE_VER1
    adinopt->sinfo.format = SF_FORMAT_RAW | SF_FORMAT_PCM_16 | SF_ENDIAN_BIG;
#else
    adinopt->sinfo.pcmbitwidth = 16;
    adinopt->sinfo.format = SF_FORMAT_RAW | SF_FORMAT_PCM_BE;
#endif
    if ((adinopt->sp =
#ifdef HAVE_LIBSNDFILE_VER1
	 sf_open(filename, SFM_READ, &adinopt->sinfo)
	 ...
	 sf_open_read(filename, &adinopt->sinfo)
      ...
      sf_perror(adinopt->sp);
      jlog("Error: adin_sndfile: failed to open speech data: \"%s\"\n",filename);
    }
  }
  if (adinopt->sp == NULL) {		/* open failure */
    return FALSE;
  }
  /* check its format */
  if (! check_format(adinopt,&adinopt->sinfo)) {
    return FALSE;
  }
  return TRUE;
}
 ...
 */
boolean
adin_sndfile_begin(void * adinoptTHIS,char *filename)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  boolean readp;

  if (filename != NULL) {
    if (adin_sndfile_open(adinopt,filename) == FALSE) {
      jlog("Error: adin_sndfile: invalid format: \"%s\"\n", filename);
      print_format(&adinopt->sinfo);
      return FALSE;
    }
  ...
  while(readp == FALSE) {
    ...
    if (adin_sndfile_open(adinopt,adinopt->speechfilename) == FALSE) {
 ...
 */
int
adin_sndfile_read(void * adinoptTHIS,SP16 *buf, int sampnum)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  int cnt;

  cnt = sf_read_short(adinopt->sp, buf, sampnum);
  if (cnt == 0) {		/* EOF */
    return -1;
  } else if (cnt < 0) {		/* error */
    sf_perror(adinopt->sp);
 ...
 */
boolean
adin_sndfile_end(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  /* close files */
  if (sf_close(adinopt->sp) != 0) {
    sf_perror(adinopt->sp);
 ...
 */
char *
adin_sndfile_get_current_filename(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
