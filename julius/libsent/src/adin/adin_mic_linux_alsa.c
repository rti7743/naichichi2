...
struct adinopt_alsa {
	...
	snd_pcm_t *handle;	///< Audio handler
	char pcm_name[MAXPATHLEN]; ///< Name of the PCM device
	int latency;	///< Lantency time in msec.  You can override this value by specifying environment valuable "LATENCY_MSEC".
	boolean need_swap;	///< Whether samples need byte swap
	int count_nodata ; ///< no data fragment after 300 msec? count.

	#if (SND_LIB_MAJOR == 0)
	struct pollfd *ufds;	///< Poll descriptor
	int count;		///< Poll descriptor count
	#endif
};
typedef struct adinopt_alsa ADINOPT;
 ...
 */
static boolean
adin_alsa_open(void * adinoptTHIS,char *devstr)
{
  ...
  if ((err = snd_pcm_open(&adinopt->handle, devstr, SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK)) < 0) {
    jlog("Error: adin_alsa: cannot open PCM device \"%s\" (%s) %d\n", devstr, snd_strerror(err),err);
    return(FALSE);
  }
  /* set device to non-block mode */
  if ((err = snd_pcm_nonblock(adinopt->handle, 1)) < 0) {
  ...
  if ((err = snd_pcm_hw_params_any(adinopt->handle, hwparams)) < 0) {
    jlog("Error: adin_alsa: cannot initialize PCM device parameter structure (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  }

  /* set interleaved read/write format */
  if ((err = snd_pcm_hw_params_set_access(adinopt->handle, hwparams, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
    jlog("Error: adin_alsa: cannot set PCM device access mode (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  }
  ...
  if ((err = snd_pcm_hw_params_set_format(adinopt->handle, hwparams, SND_PCM_FORMAT_S16_BE)) >= 0) {
    adinopt->need_swap = FALSE;
  } else if ((err = snd_pcm_hw_params_set_format(adinopt->handle, hwparams, SND_PCM_FORMAT_S16_LE)) >= 0) {
    adinopt->need_swap = TRUE;
  } else {
    jlog("Error: adin_alsa: cannot set PCM device format to signed 16bit (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  }
#else  /* LITTLE ENDIAN */
  /* try little endian, then big endian with byte swap */
  if ((err = snd_pcm_hw_params_set_format(adinopt->handle, hwparams, SND_PCM_FORMAT_S16_LE)) >= 0) {
    adinopt->need_swap = FALSE;
  } else if ((err = snd_pcm_hw_params_set_format(adinopt->handle, hwparams, SND_PCM_FORMAT_S16_BE)) >= 0) {
    adinopt->need_swap = TRUE;
  } else {
    jlog("Error: adin_alsa: cannot set PCM device format to signed 16bit (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  ...
  if ((err = snd_pcm_hw_params_set_channels(adinopt->handle, hwparams, 1)) < 0) {
    jlog("Error: adin_alsa: cannot set PCM channel to %d (%s) %d\n", 1, snd_strerror(err),err);
    return(FALSE);
  }
  
  /* set sample rate (if the exact rate is not supported by the hardware, use nearest possible rate */
#if (SND_LIB_MAJOR == 0)
  actual_rate = snd_pcm_hw_params_set_rate_near(adinopt->handle, hwparams, adinopt->srate, &dir);
  if (actual_rate < 0) {
    jlog("Error: adin_alsa: cannot set PCM device sample rate to %d (%s) %d\n", adinopt->srate, snd_strerror(actual_rate),err);
    return(FALSE);
  }
#else
  actual_rate = adinopt->srate;
  err = snd_pcm_hw_params_set_rate_near(adinopt->handle, hwparams, &actual_rate, &dir);
  if (err < 0) {
    jlog("Error: adin_alsa: cannot set PCM device sample rate to %d (%s) %d\n", adinopt->srate, snd_strerror(err),err);
    return(FALSE);
  }
#endif
  if (actual_rate != adinopt->srate) {
  ...
  /* set period size */
  {
...
#if (SND_LIB_MAJOR == 0)
    periodsize = actual_rate * adinopt->latency / 1000 * sizeof(SP16);
    if (periodsize < minsize) {
      jlog("Stat: adin_alsa: PCM latency of %d ms (%d bytes) too small, use device minimum %d bytes\n", adinopt->latency, periodsize, minsize);
      periodsize = minsize;
    } else if (periodsize > maxsize) {
      jlog("Stat: adin_alsa: PCM latency of %d ms (%d bytes) too large, use device maximum %d bytes\n", adinopt->latency, periodsize, maxsize);
      periodsize = maxsize;
    }
    actual_size = snd_pcm_hw_params_set_period_size_near(adinopt->handle, hwparams, periodsize, &dir);
    if (actual_size < 0) {
      jlog("Error: adin_alsa: cannot set PCM record period size to %d (%s) %d\n", periodsize, snd_strerror(actual_size),actual_size);
      return(FALSE);
    }
    if (actual_size != periodsize) {
      jlog("Stat: adin_alsa: PCM period size: %d bytes (%dms) -> %d bytes\n", periodsize, adinopt->latency, actual_size);
    }
...
#else
    period_time = adinopt->latency * 1000;
    if (!force && has_current_period && period_time > period_time_current) {
	jlog("Stat: adin_alsa: current latency (%dms) is shorter than %dms, leave it\n", period_time_current / 1000, adinopt->latency);
	period_time = period_time_current;
    } else {
      if ((err = snd_pcm_hw_params_set_period_time_near(adinopt->handle, hwparams, &period_time, 0)) < 0) {
	jlog("Error: adin_alsa: cannot set PCM record period time to %d msec (%s) %d\n", period_time / 1000, snd_strerror(err),err);
	return(FALSE);
      }
    ...
    if ((err = snd_pcm_hw_params_set_periods(adinopt->handle, hwparams, sizeof(SP16), 0)) < 0) {
  ...
  if ((err = snd_pcm_hw_params(adinopt->handle, hwparams)) < 0) {
    jlog("Error: adin_alsa: cannot set PCM hardware parameters (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  }

  /* prepare for recording */
  if ((err = snd_pcm_prepare(adinopt->handle)) < 0) {
    jlog("Error: adin_alsa: failed to prepare audio interface (%s) %d\n", snd_strerror(err),err);
  }

#if (SND_LIB_MAJOR == 0)
  /* prepare for polling */
  adinopt->count = snd_pcm_poll_descriptors_count(adinopt->handle);
  if (adinopt->count <= 0) {
    jlog("Error: adin_alsa: invalid PCM poll descriptors count\n");
    return(FALSE);
  }
  adinopt->ufds = mymalloc(sizeof(struct pollfd) * adinopt->count);
  if ((err = snd_pcm_poll_descriptors(adinopt->handle, adinopt->ufds, adinopt->count)) < 0) {
    jlog("Error: adin_alsa: unable to obtain poll descriptors for PCM recording (%s) %d\n", snd_strerror(err),err);
    return(FALSE);
  }
#endif

  /* output status */
  output_card_info(devstr, adinopt->handle);
 ...
 */
boolean
adin_alsa_begin(void * adinoptTHIS,char *pathname)
{
#ifndef HAS_ALSA
  return FALSE;
#else
  ...
  if (pathname != NULL) {
    strncpy(adinopt->pcm_name, pathname, MAXPATHLEN);
    jlog("Stat: adin_alsa: device name from argument: \"%s\"\n", adinopt->pcm_name);
  } else if ((p = getenv("ALSADEV")) != NULL) {
    strncpy(adinopt->pcm_name, p, MAXPATHLEN);
    jlog("Stat: adin_alsa: device name from ALSADEV: \"%s\"\n", adinopt->pcm_name);
  } else {
    strcpy(adinopt->pcm_name, "default");
  }
  /* open the device */
  if (adin_alsa_open(adinoptTHIS,adinopt->pcm_name) == FALSE) {
    jlogreport(503);
    return FALSE;
  }
  ...
  while(1) {			/* wait till prepared */
    status = snd_pcm_state(adinopt->handle);
    switch(status) {
    case SND_PCM_STATE_PREPARED: /* prepared for operation */
      if ((err = snd_pcm_start(adinopt->handle)) < 0) {
 ...
 */
boolean
adin_alsa_end(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  int err;

  if ((err = snd_pcm_close(adinopt->handle)) < 0) {
 ...
 */
int
adin_alsa_read(void * adinoptTHIS,SP16 *buf, int sampnum)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
#ifndef HAS_ALSA
  return -2;
  ...
  while ((avail = snd_pcm_avail_update(adinopt->handle)) <= 0) {
    usleep(adinopt->latency * 1000);
  }
  if (avail < sampnum) {
    cnt = snd_pcm_readi(adinopt->handle, buf, avail);
  } else {
    cnt = snd_pcm_readi(adinopt->handle, buf, sampnum);
  }
  ...
  ret = snd_pcm_wait(adinopt->handle, MAXPOLLINTERVAL);
  switch (ret) {
  case 0:			/* timeout */
    adinopt->count_nodata ++;
    jlog("Warning: adin_alsa: no data fragment after %d msec?\n", MAXPOLLINTERVAL);
    if (adinopt->count_nodata >= 3)
    {
       jlogreport(501);
       adinopt->count_nodata = 0;
    }
    cnt = 0;
    break;
  case 1:			/* has data */
    adinopt->count_nodata = 0;
    cnt = snd_pcm_readi(adinopt->handle, buf, sampnum); /* read available (non-block) */
    break;
  case -EPIPE:			/* pipe error */
    /* try to recover the broken pipe */
    snd_pcm_status_alloca(&status);
    if ((res = snd_pcm_status(adinopt->handle, status))<0) {
    ...
    if (snd_pcm_status_get_state(status) == SND_PCM_STATE_XRUN) {
      ...
      if ((res = snd_pcm_prepare(adinopt->handle))<0) {
 ...
 */
char *
adin_alsa_input_name(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
#ifndef HAS_ALSA
  return NULL;
#else
  return(adinopt->pcm_name);
#endif
}
