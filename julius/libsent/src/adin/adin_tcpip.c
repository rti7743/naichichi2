...
struct adinopt_tcpip {
	int adinnet_sd;	///< Listen socket for adinserv
	int adinnet_asd;	///< Accept socket for adinserv
	#ifdef FORK_ADINNET
	pid_t child;		/* child process ID (0 if myself is child) */
	#endif
};
typedef struct adinopt_tcpip ADINOPT;
...
adin_tcpip_standby(int freq, void *port_str)
{
  ...
  adinopt->adinnet_sd = -1;
  adinopt->adinnet_asd = -1;

  port = atoi((char *)port_str);

  if ((adinopt->adinnet_sd = ready_as_server(port)) < 0) {
 ...
 */
boolean
adin_tcpip_begin(void * adinoptTHIS,char *pathname)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
    ...
    for (;;) {
      ...
      if ((adinopt->adinnet_asd = accept_from(adinopt->adinnet_sd)) < 0) {
	return FALSE;
      }
      jlog("Stat: adin_tcpip: connected\n");
      /* fork self */
      adinopt->child = fork();
      if (adinopt->child < 0) {		/* error */
	jlog("Error: adin_tcpip: fork failed\n");
	return FALSE;
      }
      /* child thread should handle this request */
      if (adinopt->child == 0) {		/* child thread */
	break;			/* proceed */
      } else {			/* parent thread */
 ...
 */
boolean
adin_tcpip_end(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
    /* end of connection */
    close_socket(adinopt->adinnet_asd);
#ifdef FORK_ADINNET
 ...
 */
int
adin_tcpip_read(void * adinoptTHIS,SP16 *buf, int sampnum)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  int cnt, ret;
  fd_set rfds;
  ...
  FD_ZERO(&rfds);
  FD_SET(adinopt->adinnet_asd, &rfds);
  tv.tv_sec = 0;
  tv.tv_usec = 10000;		/* 10msec */
  status = select(adinopt->adinnet_asd+1, &rfds, NULL, NULL, &tv);
  if (status < 0) {		/* error */
  ...
  if (status > 0) {		/* there are some data */
    ...
    ret = rd(adinopt->adinnet_asd, (char *)buf, &cnt, sampnum * sizeof(SP16));
    if (ret == 0) {
      /* end of segment mark */
      return -3;
    }
 ...
 */
boolean
adin_tcpip_send_pause(void * adinoptTHIS)
{
   ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
   int count;
   char com;
   ...
   com = '0';
   count = wt(adinopt->adinnet_asd, &com, 1);
 ...
 */
boolean
adin_tcpip_send_resume(void * adinoptTHIS)
{
  ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
  int count;
  char com;
  ...
  do {
    ...
    if (status > 0) {		/* there are some data */
      ...
      ret = rd(adinopt->adinnet_asd, tmpbuf, &cnt, MAXSPEECHLEN * sizeof(SP16));
    }
    if (cnt > 0) count += cnt;
  } while (status != 0);
  if (count > 0) {
 ...
 */
boolean
adin_tcpip_send_terminate(void * adinoptTHIS)
{
   ADINOPT* adinopt = (ADINOPT*)adinoptTHIS;
   int count;
   char com;
 ...
 */
char *
adin_tcpip_input_name(void * adinoptTHIS)
{
