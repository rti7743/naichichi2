 ...
 *
 * <JA>
 * @brief  ²»À¼¶è´Ö¸¡½Ð¤Î¤¿¤á¤ÎÎúÔòº¹¿ô¥«¥¦¥ó¥È
 *
 * Í¿¤¨¤éŽ¤¡¦¿¥Ð¥Ã¥Õ¥¡Ä¹Æâ¤ÎÎúÔòº¹¿ô¤ò¥«¥¦¥ó¥È¤·¤Þ¤¹¡Ž¥
 * Æ±»þ¤Ë, ¸Æ¤ÐŽ¤¡¦¿¥Ð¥Ã¥Õ¥¡¤ò½ç¼¡¥Ð¥Ã¥Õ¥¡Ä¹Ê¬¤À¤±¸Å¤¤¤â¤Î¤ËÆþ¤¡¦Ø¤¨¤Þ¤¹¡Ž¥
 * ¤³¤Î¤¿¤áÆþÎÏ¤Ï¥Ð¥Ã¥Õ¥¡Ä¹Ê¬¤À¤±ÃÙ±ä¤¹Ž¤¡¦³¤È¤Ë¤Ê¤ê¤Þ¤¹¡Ž¥
 * </JA>
 * <EN>
...
{
	int trigger=zc->trigger;
	if (isUpOrDown < 0)
	{/*down*/
		if (trigger > zc->triggerbase)
		{
			trigger = zc->triggerbase;
		}
		else
		{
			trigger -= 500;
			if(trigger < zc->triggermin)
			{
				trigger = zc->triggermin;
			}
		}
	}
	else
	{/*up*/
		if ( zc->level > zc->triggermax )
		{
			trigger = zc->triggermax;
		}
		else if (zc->level > zc->triggerbase)
		{
			trigger = zc->level;
		}
		else
	...
	if (zc->trigger == trigger)
	{/*no change*/
		return ;
	}
	
	zc->trigger = trigger;
    jlog("change_count_zc_e %d ,zc->level:%d  zc->trigger:%d\n",isUpOrDown,zc->level,zc->trigger);
}
 ...
 */
void
reset_count_zc_e(ZEROCROSS *zc, int c_trigger,int c_triggermin,int c_triggermax, int c_length, int c_offset)
{
  int i;
  ...
  zc->triggerbase = c_trigger;
  zc->triggermin  = c_triggermin;
  zc->triggermax  = c_triggermax;
  zc->offset = c_offset;
