...
namespace {
float toLogProb(float f1, float f2) {
...
class HMMLearner {
 public:
  static int run(Param *param) {
    ...
    std::map<std::string, std::map<std::string, float> > emission;
    std::map<std::string, std::map<std::string, float> > transition;
    ...
    // dictionary
    {
      ...
      const float freq = param->get<float>("default-emission-freq");
...

    {
      ...
      // bigram
      for (std::map<std::string, std::map<std::string, float> >
               ::const_iterator
               it = transition.begin();
           it != transition.end(); ++it) {
        float freq = 0.0;
        for (std::map<std::string, float>::
    ...
    for (size_t i = 0; i < 10; ++i) {
      ...
      while (ifs) {
        ...
        for (size_t k = 0; k < feature_index.size(); ++k) {
          const float tmp = (observed[k] - expected[k]);
          margin += alpha[k] * tmp;
          s += tmp * tmp;
        }
        ...
        if (s > 0.0) {
          const float diff = _max(0.0f, 10 - margin) / s;
          if (diff > 0.0) {
            for (size_t k = 0; k < feature_index.size(); ++k) {
 ...
 public:
  static int run(Param *param) {
    ...
    for (size_t itr = 0; ;  ++itr) {
      ...
      float diff = (itr == 0 ? 1.0 : std::fabs(1.0 *(old_f - f) )/old_f);
      std::cout << "iter="    << itr
                << " err="    << 1.0 * err/x_.size()
                << " F="      << micro_f
                << " target=" << f
