...
bool FeatureIndex::convert(const char* txtfile, const char *binfile) {
  ...
  char buf[BUF_SIZE];
  char *column[4];
  std::map<std::string, float> dic;

  while (ifs.getline(buf, sizeof(buf))) {
    CHECK_DIE(tokenize2(buf, "\t", column, 2) == 2)
        << "format error: " << buf;

    dic.insert(std::make_pair<std::string, float>
  ...
  for (std::map<std::string, float>::const_iterator
    ...
    ofs.write(reinterpret_cast<const char*>(&it->second), sizeof(float));
  }

  Darts::DoubleArray da;
  CHECK_DIE(da.build(key.size(), &key[0], 0, 0, 0) == 0)
      << "unkown error in building float array: " << binfile;
